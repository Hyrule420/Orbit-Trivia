...
export function usePositionSource(route) {
  ...
  /* The raw code from the browser: 1 = permission denied, 2 = position
     unavailable, 3 = timed out. Kept so the screen can tell the user
     which of those actually happened instead of a vague "location off". */
  ...
  const startGps = useCallback(() => {
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      ...
      (err) => {
        ...
        if (err.code === 1) {
          /* PERMISSION_DENIED — the browser or the OS is refusing. This
             is the only one worth giving up on. */
          setStatus("denied");
          setError("This site isn't allowed to see your location.");
          clearWatch();
        } else if (err.code === 2) {
          /* POSITION_UNAVAILABLE — permission is fine, but the phone
             cannot actually work out where it is. On an iPhone the
             usual cause is Airplane Mode, which switches the GPS radio
             off while leaving Wi-Fi up, so the page still loads. */
          setStatus("unavailable");
          setError("Your phone can't get a fix right now.");
        } else {
          /* TIMEOUT — a tunnel, a garage, a bad patch of sky. Keep the
             watch running; this one genuinely does recover by itself. */
          setStatus("searching");
          setError(null);
        }
      },
