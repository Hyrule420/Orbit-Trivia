/* ============================================================
   ROAD TRIP FLORIDA — NATURE COAST PACK

   This is the content for the GPS mode. Everything the road trip
   knows about the world is in this one file.

   ------------------------------------------------------------
   HOW TO ADD A PLACE
   ------------------------------------------------------------
   1. Open Google Maps and find the spot on US-19 that is NEAREST
      the landmark — not the landmark itself. Right-click the road,
      and Google copies the two numbers (latitude, longitude) for you.
   2. Paste them in as `lat` and `lng`.
   3. Write the question. Copy the shape of any entry below.
   4. Save. That's it — no database, no API key, nothing to deploy.

   ------------------------------------------------------------
   THE RULES (breaking these is how zones silently never fire)
   ------------------------------------------------------------
   * PUT THE PIN ON THE HIGHWAY. If you centre a zone on a building
     three miles off the road, a 2.5 km circle never touches US-19 and
     the question can never trigger. The `blurb` is where you say
     "off to your west..." — the pin's job is only to catch the car.
   * radiusM between 1500 and 3000. At 60 mph a 2,500 m circle gives
     you about three minutes inside the zone. Plenty.
   * KEEP ZONES 4 km APART. Otherwise the towns fire five questions in
     a row and the quiet stretches have nothing at all. Pacing is the
     whole game here.
   * `d` (difficulty) must be exactly "Earthbound", "Orbit" or
     "Martian" — that's what decides points and colour.
   * `o` must have exactly 4 options and `a` must be one of them,
     spelled identically.
   * `id` must be unique and must never be reused or renamed. Saved
     progress is stored against it — rename an id and the game forgets
     you answered it. This is why a place squeezed in between two
     existing ones gets a letter rather than a new number: inserting
     "nc-02a" between nc-02 and nc-03 keeps this list in geographic
     order without renumbering anything that people have already played.

   There is an automatic check for most of these. Run the app in
   development and look at the browser console.
   ============================================================ */
...
export const CORRIDOR = {
  ...
  name: "Nature Coast",
  road: "US-19",
  /* Roughly Fanning Springs down to Tarpon Springs. Used to fit the map
     and to catch zones that have been typed in wrong — anything outside
     this box is almost certainly a mistyped coordinate. Remember to
     widen it whenever the route is extended. */
  bounds: [
    [28.10, -83.00],
    [29.65, -82.45],
  ],
...
/* Fast lookup by id. The queue and the saved progress store ids only —
   never whole question objects — so that editing the wording of a
   question can never corrupt somebody's saved game. */
export const GEO_BY_ID = Object.fromEntries(GEO_QUESTIONS.map((z) => [z.id, z]));
