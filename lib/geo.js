...
export function checkPack(zones, route, bounds, maxDistanceFromRouteM = 6000) {
  ...
  zones.forEach((z) => {
    ...
    /* A zone carries either one question inline or several in a
       `questions` array — see zoneQuestions() in lib/corridors/index.js.
       Both shapes get checked the same way, and every entry is checked,
       because a broken answer buried at index 3 is exactly the kind of
       thing nobody notices until a player hits it. */
    const asked = Array.isArray(z.questions) && z.questions.length
      ? z.questions
      : (z.q ? [{ q: z.q, o: z.o, a: z.a }] : []);

    if (!asked.length) {
      problems.push(`${z.id}: has no question at all`);
    }
    asked.forEach((entry, i) => {
      const where = asked.length > 1 ? `question ${i + 1}` : "its question";
      if (!entry.q) problems.push(`${z.id}: ${where} has no text`);
      if (entry.o?.length !== 4) {
        problems.push(`${z.id}: ${where} has ${entry.o?.length} options, expected 4`);
      }
      if (!entry.o?.includes(entry.a)) {
        problems.push(`${z.id}: ${where} — the answer "${entry.a}" is not one of its options`);
      }
      if (entry.o && new Set(entry.o).size !== entry.o.length) {
        problems.push(`${z.id}: ${where} lists the same option twice`);
      }
    });
    if (z.real !== undefined) {
      const ok = Array.isArray(z.real) && z.real.length === 2 &&
        typeof z.real[0] === "number" && typeof z.real[1] === "number";
      if (!ok) {
        problems.push(`${z.id}: real is present but isn't a [lat, lng] pair`);
      } else {
        const [rLat, rLng] = z.real;
        if (rLat < minLat || rLat > maxLat || rLng < minLng || rLng > maxLng) {
          problems.push(`${z.id}: real (${rLat}, ${rLng}) is outside the corridor bounds`);
        }
        const off = distanceM(z.lat, z.lng, rLat, rLng);
        if (off < 150) {
          problems.push(`${z.id}: real is only ${Math.round(off)} m from the pin — not worth a leader line`);
        }
      }
    }
  });
