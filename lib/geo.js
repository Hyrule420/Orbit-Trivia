...
export function checkPack(zones, route, bounds, maxDistanceFromRouteM = 6000) {
  ...
  /* Zones packed too tightly fire on top of each other. The condition
     that actually matters is whether their circles overlap, so compare
     against the two radii rather than a fixed distance — that way the
     check stays right when a zone is deliberately made smaller, as the
     in-town ones south of Hudson are. */
  for (let i = 0; i < zones.length; i++) {
    for (let j = i + 1; j < zones.length; j++) {
      const d = distanceM(zones[i].lat, zones[i].lng, zones[j].lat, zones[j].lng);
      const overlapAt = (zones[i].radiusM || 0) + (zones[j].radiusM || 0);
      if (d < overlapAt) {
        problems.push(
          `${zones[i].id} and ${zones[j].id} overlap — ${Math.round(d)} m apart but ${overlapAt} m of combined radius`
        );
      }
    }
  }
