...
/* Is today a "Heavy" day — the flavour where the Road Trip landing zones
   get two boosters coming home at once and the double sonic boom that
   goes with them?

   Derived from todaySeed() rather than stored, which buys three things:
   everyone driving the Space Coast on the same day sees the same thing,
   it needs no storage and cannot drift, and it is trivially testable.
   Roughly one day in five, which keeps it a treat rather than a habit. */
export const heavyDay = () => todaySeed() % 5 === 0;
